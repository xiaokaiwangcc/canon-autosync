...
- **文件浏览**：已备份 / 待备份双标签（每 5s 自动刷新），列表/图标两种视图，支持文件名搜索、缩略图封面（RAW 提取内嵌 JPEG 预览、MP4/MOV 提取视频首帧，其余不可预览格式显示类型占位图标）、大图预览（键盘 ← → 切换、Esc 关闭）、视频可在线播放
...
- 安装向导询问相机 IP 和备份目录；使用自定义目录需在 应用设置 → 授权目录中授权
